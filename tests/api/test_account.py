...
def test_valid_get_info_about_user(log_test, register_client, mail_client, auth_client, account_client, reg_auth_data,
    ...
    with allure.step("Получаем информацию о пользователе"):
        response = account_client.get_account_info(auth_token=user_auth_token)
...
def test_invalid_get_info_about_user(log_test, register_client, mail_client, auth_client, account_client,
                                     reg_auth_data, user_auth_token):
    with allure.step("Получаем информацию о пользователе с некорректным токеном авторизации"):
        response = account_client.get_account_info(auth_token=user_auth_token + "1")
    with allure.step("Проверяем статус-код и тело ответа"):
...
@allure.title("Позитивная проверка изменения информации о пользователе в одном поле {field}")
...
def test_valid_change_info_about_user_in_one_field(log_test, register_client, mail_client, auth_client, account_client,
                                                   reg_auth_data, user_data, field, user_auth_token):
    with allure.step("Изменяем информацию о пользователе"):
        payload = {field: user_data[field]}
        response = account_client.change_account_info(auth_token=user_auth_token, payload=payload)
    with allure.step("Проверяем статус-код и тело ответа"):
...
def test_valid_change_info_about_user_in_all_fields(log_test, register_client, mail_client, auth_client, account_client,
                                                    reg_auth_data, user_data, user_auth_token):
    with allure.step("Изменяем информацию о пользователе"):
        payload = user_data
        response = account_client.change_account_info(auth_token=user_auth_token, payload=payload)
    with allure.step("Проверяем статус-код и тело ответа"):
...
def test_valid_change_password(log_test, register_client, mail_client, auth_client, account_client,
    ...
    with allure.step("Предусловие: регистрация и активация пользователя"):
        ...
        with allure.step("Авторизовываемся только что созданным и активированным пользователем"):
            payload = {
    ...
    with allure.step("Меняем пароль пользователю"):
        with allure.step("Сбрасываем пароль, используя логин и email"):
            account_client.reset_password(login=reg_auth_data["login"], email=reg_auth_data["email"])
        with allure.step("Находим письмо по сбросу пароля и забираем оттуда токен"):
            token = mail_client.find_reset_password_letter_by_login(login=reg_auth_data["login"])
        with allure.step("Меняем пароль, используя взятый токен"):
        ...
        with allure.step("Попытка авторизоваться со старым паролем"):
            payload = {
                "login": reg_auth_data["login"],
                "password": reg_auth_data["password"],
                "rememberMe": True
        ...
        with allure.step("Попытка авторизоваться с новым паролем"):
            payload = {
                "login": reg_auth_data["login"],
...
@allure.title("Позитивная проверка удаления пользователя")
@allure.description("Проверяем удаление пользователя через API")
def test_valid_delete_account(log_test, register_client, mail_client, auth_client, account_client,
                              reg_auth_data, user_data, user_auth_token):
    with allure.step("Запрашиваем письмо для получения токена удаления"):
        account_client.request_for_deleting_account(email=reg_auth_data["email"], auth_token=user_auth_token)
    with allure.step("Забираем токен удаления из письма"):
        delete_token = mail_client.find_delete_user_letter_by_login(reg_auth_data["login"])
    with allure.step("Удаляем пользователя"):
        response = account_client.delete_account(delete_token=delete_token, auth_token=user_auth_token)
        with allure.step("Проверяем статус-код ответа"):
            assert response.status_code == 204
    with allure.step("Попытка авторизоваться удаленным пользователем"):
        payload = {
            "login": reg_auth_data["login"],
            "password": reg_auth_data["password"],
            "rememberMe": True
